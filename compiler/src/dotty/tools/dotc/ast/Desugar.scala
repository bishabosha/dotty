  ...
  def apply(tree: Tree)(implicit ctx: Context): Tree = {
    ...
    val desugared = tree match {
      ...
      case TypedNumber(digits, kind, fromDigits) =>
        ...
        val fromDigitsPath = Select(TypedSplice(tpd.ref(defn.NumericContextModule)), fromDigits).withSpan(span)
        val givenName = UniqueName.fresh(termName(s"given_$fromDigits"))
        val givenAlias = DefDef(givenName, Nil, Nil, SingletonTypeTree(fromDigitsPath), fromDigitsPath)
        givenAlias.setMods(Modifiers(Given | Inline))
        val numberType = AppliedTypeTree(TypeTree(defn.NumericContext_NumberType.typeRef), SingletonTypeTree(Ident(givenName)) :: Nil)
        val summoned = TypeApply(ref(fromDigitsSummoner.namedType), numberType::Nil)
        return Block(givenAlias :: Nil, summoned.applyFromDigits(digits, kind))
      case PostfixOp(t, op) =>
